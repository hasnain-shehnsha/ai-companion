...
async def test_register_with_valid_timezone(async_client, db_session):
    ...
    # Verify in DB
    result = await db_session.execute(select(User).where(User.email == "tz_register@example.com"))
...
async def test_update_timezone_via_patch(async_client, db_session):
    ...
    # Verify in DB
    result = await db_session.execute(select(User).where(User.email == "tz_update@example.com"))
    user = result.scalar_one()
    assert user.timezone == "America/New_York"


async def test_reminder_timezone_conversion(async_client, mocker, db_session):
    # Register user with Asia/Karachi (UTC+5)
    ...
    # e.g. the user says "remind me at 9am tomorrow", the LLM parses it to 9am.
    class MockMessage:
        content = '{"intent": "SET_REMINDER", "datetime_iso": "2030-01-01T09:00:00", "reminder_text": "Wake up", "is_general_question": false, "search_query": null}'
    ...
    mocker.patch("app.services.intent_service.client.chat.completions.create", return_value=MockResponse())
    
    chat_response = await async_client.post(
        "/chat/",
        json={"message": "remind me to wake up at 9am tomorrow"},
        headers={"Authorization": f"Bearer {token}"}
    )
    assert chat_response.status_code == 200
    
    # Verify DB reminder time is correctly offset to UTC
    # 09:00:00 in Asia/Karachi (UTC+5) is 04:00:00 in UTC
    result = await db_session.execute(select(User).where(User.email == "tz_reminder@example.com"))
    user = result.scalar_one()
    
    rem_result = await db_session.execute(select(Reminder).where(Reminder.user_id == user.id))
    reminder = rem_result.scalar_one()
    
    # remind_at should be exactly 2030-01-01 04:00:00+00:00
    assert reminder.remind_at.hour == 4
    assert reminder.remind_at.minute == 0
    assert reminder.remind_at.tzinfo == datetime.timezone.utc
